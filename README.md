...
The project includes a powerful code generation system to automate the creation of boilerplate for apps, entities, and other components. These commands are available under the `make` group.

**`python manage.py make:app <name>`**

Creates a new modular app inside the `apps/` directory with a default structure.

*   `<name>`: The name of the new app (e.g., `products`).

This command generates the basic directory structure for an app, including subdirectories for models, repositories, services, schemas, routers, permissions, and tests.

**`python manage.py make:entity <name> --app <app>`**

Creates a new complete entity (model, schema, repository, service, router, permission, and test) inside an existing app.

*   `<name>`: The name of the entity (e.g., `post`).
*   `--app <app>`: The app where the entity will be created (required).
*   `--only <components>`: Generate only specific components (comma-separated: `model`, `schema`, `repository`, `service`, `router`, `permission`, `test`).
*   `--except <components>`: Generate all components except the specified ones.
*   `--minimal`: A shortcut for generating only `model` and `schema`.
*   `--table-name <name>`: Override the default table name.
*   `--no-prefix`: Disable the app name prefix in the table name.

Example:
```bash
# Creates a full 'post' entity in the 'blog' app
python manage.py make:entity post --app blog

# Creates only the model and schema for a 'category' entity
python manage.py make:entity category --app products --minimal
```
...
*   `<name>`: The name of the new command file (e.g., `reports`).

**`python manage.py make:middleware <name>`**

Creates a new middleware file in the `core/middlewares/` directory.

*   `<name>`: The name of the new middleware (e.g., `profiling`).

### Code Removal (`rm`)

Commands to safely remove apps or entities generated by the `make` commands.

**`python manage.py rm:app <name>`**
Removes an entire app from the `apps/` directory.
- `<name>`: The name of the app to remove.
- `--force`: Skips the confirmation prompt.

**`python manage.py rm:entity <name> --app <app>`**
Removes an entity and its related files from an app.
- `<name>`: The name of the entity to remove (e.g., `post`).
- `--app <app>`: The name of the app containing the entity.
- `--only <components>`: Removes only specific components (comma-separated: `model`, `schema`, `repository`, `service`, `router`, `permission`, `test`).
- `--except <components>`: Removes all components except the ones specified.
- `--force`: Skips the confirmation prompt.

**`python manage.py rm:command <name>`**
Removes a custom command file from the `core/commands/` directory.
- `<name>`: The name of the command file to remove.

### Database (`db`)

**`python manage.py db:create`**
Creates all database tables based on your SQLAlchemy models.
...
- `-m "<message>"`: A descriptive message for the revision (required).
...
- `--steps <n>`: The number of migrations to rollback (default: 1).

**`python manage.py db:reset`**
Resets the database (drops all tables and applies all migrations). Asks for confirmation.
